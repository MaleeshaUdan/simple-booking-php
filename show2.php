...
$bookedSeats = array();
$query = "SELECT bookingNo FROM bookings where showNo =2 ";
...
<div class="header">
  ...
  <h2>Welcome to Short Film Show 2</h2>
</div>
...
<?php
    if(isset($_POST['seatNumber']) && !empty($_POST['seatNumber'])){
        ...
        $sql ="SELECT * From bookings where userId= $user_id AND showNo='1'";
        ...
        elseif($canBook=="False"){
            echo "<p>You already booked a seat, the number is " . $prev_show_seat_no . "in the show 1</p>";
        }
        else{
            $sql2 = "INSERT INTO bookings (userId, showNo, bookingNo) VALUES ('$user_id', 2, '$seatNumber')";
            if ($conn->query($sql2) === TRUE) {
                echo "<p>Your seat number is " . $seatNumber . "in the show 2</p>";
            } else {
